...
//  3D Parallax Slider -------------------------------------------------------------------------------------------------
    ...
    $(document).on("touchstart", ".outer-wrapper", function(e) {
        if ($body.hasClass("zoomed-out")) {
            e.preventDefault(); // Prevent default touch behavior
            $dragging = true;
            var touch = e.originalEvent.touches[0];
            firstClickPositionX = touch.pageX;
            firstClickPositionY = touch.pageY;
            pageWrapperLastPositionX = moveX;
            pageWrapperLastPositionY = moveY;
            $(".inner-wrapper").css("transition", "0s");
        }
    ...
    $(document).on("touchmove", function(e) {
        if ($body.hasClass("zoomed-out") && $dragging) {
            e.preventDefault(); // Prevent default touch behavior
            var touch = e.originalEvent.touches[0];
            moveX = pageWrapperLastPositionX + (touch.pageX - firstClickPositionX);
            moveY = pageWrapperLastPositionY + (touch.pageY - firstClickPositionY);
            $(".inner-wrapper").css("transform", "translateZ(-4000px) translateX(" + moveX + "px) translateY(" + moveY + "px)");
        }
    });
